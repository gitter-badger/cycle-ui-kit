...
gulp.task('info', (done) => {
  ...
  const makingChangesList = new Listr([
      ...
      {
        title: `Keep ${chalk.white.underline('gulp lint --watch')} running and make sure there aren\'t any errors, and little to no warnings.`,
        task: () => Promise.resolve()
      },
      {
        title: `Use ${chalk.white.underline('gulp test')} to run the tests. You can pass in the ${chalk.white.underline('--watch')} option too.`,
        task: () => Promise.resolve()
      },
      {
  ...
  makingChangesList.run().then(() => {
    ...
    const finishingList = new Listr([
        {
          title: `Run ${chalk.white.underline('gulp greenlight')} and make sure everything\'s going swell.`,
          task: () => Promise.resolve()
        },
        {
